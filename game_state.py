# game_state.py
# Серверная логика игры SkyStrike
# Версия 0.6
# Добавлен счёт побед команд, улучшена обработка таймаутов.
...
class GameState:
    ...
    def add_player(self, name: str, team: str) -> Optional[Player]:
        """Добавляет нового игрока, проверяет уникальность имени."""
        ...
        self.next_id += 1
        logger.info(f"Игрок {name} ({team}) добавлен (ID {player.id})")
        return player
    ...
    def update(self, dt: float):
        """Основной цикл обновления (вызывается каждые 50 мс)."""
        now = time.time()

        # 1. Проверка таймаута (3 секунды без действия)
        ...
        if self.bomb.status == 'planted':
            ...
            if self.bomb.timer <= 0:
                ...
                self.end_round('T')  # Террористы выигрывают взрывом
            # Обезвреживание
            if self.bomb.defusing:
                ...
                if self.bomb.defuse_timer <= 0:
                    ...
                    self.end_round('CT')  # Контр-террористы обезвредили

        # 4. Обновление игроков
        for player in self.players.values():
            ...
            if inputs['mouse_dx'] != 0:
                player.angle += inputs['mouse_dx'] * 2.0 * dt * 10  # чувствительность
            ...
            if length > 0:
                ...
                if distance(player.x, player.y, new_x, new_y) > max_move * 1.5:
                    ...
                    # Коллизии со стенами и другими игроками (по осям раздельно)
            ...
            if inputs['defuse'] and player.team == 'CT' and self.bomb.status == 'planted' and player.alive:
                if not self.bomb.defusing:
                    if distance(player.x, player.y, self.bomb.x, self.bomb.y) < 1.0:
                        ...
                        self.bomb.defuse_timer = 5.0  # BOMB_DEFUSE_TIME
    ...
    def collides_with_players(self, x: float, y: float, exclude_id: int) -> bool:
        """Проверяет, пересекается ли позиция с другими живыми игроками."""
        for pid, p in self.players.items():
            if pid == exclude_id or not p.alive:
                continue
            if distance(x, y, p.x, p.y) < 0.6:  # PLAYER_RADIUS * 2
                return True
        return False

    def process_shot(self, shooter: Player):
        """Обрабатывает выстрел: луч, попадания в игроков и стены."""
    ...
    def cast_ray(self, x, y, angle):
        """Возвращает расстояние до ближайшей стены в направлении угла."""
    ...
    def get_state(self, player_id: int) -> dict:
        """Возвращает состояние для конкретного игрока (включая других игроков)."""
    ...
    def get_stats(self) -> dict:
        """Возвращает общую статистику для административных нужд."""
        return {
